<template>
  <div class="fixed inset-0 z-50 overflow-y-auto">
    ...
    <div class="relative min-h-screen flex items-center justify-center p-4">
      <div class="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
        <!-- Header -->
        <div class="sticky top-0 bg-white dark:bg-gray-800 flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700 z-10">
        ...
        <form @submit.prevent="handleSubmit" class="p-4 space-y-4">
          ...
          <!-- Validation Errors -->
          <div v-if="Object.keys(validationErrors).length > 0" class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-600 dark:text-red-400 px-3 py-2 rounded-lg text-sm">
          ...
          <div>
            ...
            <p v-if="validationErrors.teacher_id" class="text-red-500 text-xs mt-1">
              {{ Array.isArray(validationErrors.teacher_id) ? validationErrors.teacher_id[0] : validationErrors.teacher_id }}
            </p>
          </div>
          ...
          <div class="space-y-2">
            <div class="flex items-center justify-between">
              ...
              <button type="button" @click="addScheduleItem" class="text-primary-600 hover:text-primary-700 text-xs flex items-center gap-1">
            ...
            <p v-if="validationErrors.schedule" class="text-red-500 text-xs">
              {{ Array.isArray(validationErrors.schedule) ? validationErrors.schedule[0] : validationErrors.schedule }}
            </p>

            <div v-for="(item, index) in form.schedule" :key="index" class="flex items-center gap-2 p-2 bg-gray-50 dark:bg-gray-700 rounded-lg">
              <!-- Day -->
          ...
          <div class="grid grid-cols-2 gap-3">
            <label class="flex items-center gap-2 p-2 bg-gray-50 dark:bg-gray-700 rounded-lg cursor-pointer">
          ...
          <div class="flex items-center justify-end gap-3 pt-3 border-t border-gray-200 dark:border-gray-700">
            ...
            <button type="submit" :disabled="loading" class="btn-primary py-1.5 px-3 text-sm flex items-center gap-1">
