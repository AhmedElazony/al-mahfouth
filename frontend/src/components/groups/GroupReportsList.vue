<template>
	<div class="space-y-4">
		...
		<div class="flex flex-col gap-3">
			...
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
				...
				<div class="relative">
					...
					<div class="relative">
						<i
							class="pi pi-calendar absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm pointer-events-none"></i>
						<input v-model="dateFrom" type="date" class="input pr-10 w-full"
				...
				<div class="relative">
					...
					<div class="relative">
						<i
							class="pi pi-calendar absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm pointer-events-none"></i>
						<input v-model="dateTo" type="date" class="input pr-10 w-full"
				...
				<div class="relative">
					...
					<div class="relative">
						<i
							class="pi pi-search absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm pointer-events-none"></i>
						<input v-model="studentSearch" type="text" class="input pr-10 w-full"
				...
				<div class="flex flex-col justify-end gap-2 md:flex-row">
					<button @click="clearAllFilters" class="btn-secondary whitespace-nowrap"
						:disabled="!hasActiveFilters">
						<i class="pi pi-times mr-2"></i>
						<span>{{ $t('common.clear') }}</span>
					</button>

					<button @click="openAddReportModal" class="btn-primary whitespace-nowrap">
		...
		<!-- Error State -->
		<div v-else-if="error"
			class="card p-3 sm:p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800">
		...
		<div v-else-if="filteredReports.length > 0" class="space-y-3">
				...
				<div class="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
					<div class="flex-1 min-w-0">
						...
						<div class="flex flex-wrap items-center gap-1.5 sm:gap-2 mb-2 sm:mb-3">
							...
							<span v-if="report.memorized_amount"
			...
			<div v-if="totalPages > 1"
				...
				<div class="flex items-center gap-2 order-1 sm:order-2">
					...
					<button @click="previousPage" :disabled="currentPage === 1" class="btn-secondary p-2"
						:class="{ 'opacity-50 cursor-not-allowed': currentPage === 1 }">
					...
					<button @click="nextPage" :disabled="currentPage === totalPages" class="btn-secondary p-2"
		...
		<ReportFormModal v-if="showReportModal" :group="group" :students="students" :report="selectedReport"
			:default-date="dateFrom || dateTo" @close="closeReportModal" @saved="onReportSaved" />
...
async function handleDelete() {
	...
	try {
		await reportService.deleteReport(props.group.id, selectedReport.value.id)

		// ✅ Option 1: Remove from local array (faster, no refetch)
