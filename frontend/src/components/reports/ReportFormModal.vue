				...
				<form @submit.prevent="handleSubmit" class="p-6 space-y-6">
					...
					<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
						...
						<div>
							...
							<select v-model="form.student_id" class="input w-full" :disabled="isEditing"
								:class="{ 'opacity-60 cursor-not-allowed': isEditing }" required
								:selected="form.student_id">
								<option value="">{{ $t('groups.selectStudent') }}</option>
								<option v-for="student in students" :key="getStudentId(student)"
						...
						<div>
							...
							<select v-model="form.attendance_status" class="input w-full" required>
								...
								<option v-for="option in AttendanceStatusOptions" :key="option.value"
									:value="option.value">
									{{ option.label }}
								</option>
							</select>
					...
					<div>
						...
						<textarea v-model="form.notes" rows="3" class="input w-full resize-none"
							:placeholder="$t('reports.notesPlaceholder')"></textarea>
					</div>
...
interface Props {
	group: Group | { id: number, name: string } | undefined
	students: GroupStudentResponse[] | User[]
	report?: Report | null
	defaultDate?: string
}
...
// Methods
function getStudentId(student: GroupStudentResponse | User): number {
	if (typeof student.student === 'object' && 'id' in student.student) {
...
function getStudentName(student: GroupStudentResponse | User): string {
...
async function handleSubmit() {
	...
	try {
		if (isEditing.value && props.report) {
			...
			if (props.group) {
				await reportService.updateReport(props.group?.id || 0, props.report.id, payload as any)
			} else {
				await reportService.updateStandaloneReport(props.report.id, payload as any)
			}
		} else {
			...
			if (props.group) {
				await reportService.createReport(props.group?.id || 0, payload as any)
			} else {
				await reportService.createStandaloneReport(payload as any)
			}
		}
