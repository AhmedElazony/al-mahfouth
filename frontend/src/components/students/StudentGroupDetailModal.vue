    ...
    <div class="relative min-h-screen flex items-center justify-center p-4">
      <div class="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-2xl">
        ...
        <div class="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
          <div>
            <h2 class="text-xl font-bold text-gray-900 dark:text-white">{{ group.group.name }}</h2>
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="flex items-center gap-3">
              <div class="w-10 h-10 rounded-full bg-primary-100 dark:bg-primary-900 flex items-center justify-center">
                <span class="text-primary-600 dark:text-primary-400 font-semibold">
                  {{ group.group.teacher?.name?.charAt(0).toUpperCase() }}
                </span>
              </div>
              <div>
                <p class="font-medium text-gray-900 dark:text-white">{{ group.group.teacher?.name }}</p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div class="space-y-3">
              <div class="flex items-center justify-between">
                <span class="text-sm text-gray-600 dark:text-gray-400">{{ $t('students.myStatus') }}</span>
                <span 
              ...
              <div class="flex items-center justify-between">
                <span class="text-sm text-gray-600 dark:text-gray-400">{{ $t('students.memorization.amount') }}</span>
          ...
          <!-- Group Info -->
          <div>
            <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-3">{{ $t('nav.groupInfo') }}</h3>
            <div class="space-y-3">
              <div class="flex items-center justify-between">
                <span class="text-sm text-gray-600 dark:text-gray-400">{{ $t('groups.type') }}</span>
                <span 
                  :class="group.group.is_online 
                    ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400' 
                    : 'bg-gray-100 text-gray-800 dark:bg-gray-900/20 dark:text-gray-400'"
                  class="px-3 py-1 text-sm rounded-full"
                >
                  {{ group.group.is_online ? $t('groups.online') : $t('groups.offline') }}
                </span>
              </div>
              <div class="flex items-center justify-between">
                <span class="text-sm text-gray-600 dark:text-gray-400">{{ $t('common.status') }}</span>
                <span 
                  :class="group.group.is_active 
                    ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400' 
                    : 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400'"
                  class="px-3 py-1 text-sm rounded-full"
                >
                  {{ group.group.is_active ? $t('common.active') : $t('common.inactive') }}
                </span>
              </div>
            </div>
          </div>
