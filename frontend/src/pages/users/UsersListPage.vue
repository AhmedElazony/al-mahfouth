	...
	<div class="space-y-6">
		...
		<div v-else class="hidden md:block card overflow-hidden">
					...
					<tr v-for="user in usersStore.users" :key="user.id" class="hover:bg-gray-50 dark:hover:bg-gray-800">
						...
						<td class="px-6 py-4 whitespace-nowrap text-sm">
							<div class="flex items-center gap-2">
								<button @click="openEditModal(user)" :disabled="loadingUserId === user.id"
									class="p-2 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors disabled:opacity-50"
									:title="$t('common.edit')">
									<i
										:class="loadingUserId === user.id ? 'pi pi-spinner pi-spin' : 'pi pi-pencil'"></i>
								</button>
								<button @click="confirmDelete(user)"
									v-if="user.role.value !== UserRoles.SUPER_ADMIN || user.id !== authStore.user?.id"
			...
			<div v-if="usersStore.hasUsers"
				...
				<div class="flex items-center gap-2">
					<button @click="usersStore.setPage(usersStore.currentPage - 1)"
						:disabled="usersStore.currentPage === 1" class="btn-secondary px-3 py-1 disabled:opacity-50">
		...
		<div v-if="!usersStore.loading && !usersStore.error" class="md:hidden space-y-4">
			...
			<!-- User Cards -->
			<div v-for="user in usersStore.users" :key="user.id" class="card p-4 space-y-3">
			...
			<div v-if="usersStore.hasUsers" class="card p-4 flex flex-col gap-4">
				...
				<div class="flex items-center justify-center gap-4">
					<button @click="usersStore.setPage(usersStore.currentPage - 1)"
						:disabled="usersStore.currentPage === 1" class="btn-secondary px-4 py-2 disabled:opacity-50">
		...
		<ConfirmModal v-if="showDeleteModal" :title="$t('users.deleteUser')"
			:message="$t('users.deleteConfirmation', { name: userToDelete?.name })" :confirm-text="$t('common.delete')"
			:cancel-text="$t('common.cancel')" variant="danger" @confirm="handleDelete"
...
import { UserRole as UserRoles } from '@/constants'
