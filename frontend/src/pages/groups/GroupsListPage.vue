	...
	<div class="space-y-6">
		...
		<div class="card p-4">
			<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
				<div class="flex-1 min-w-[200px]">
					<input v-model="searchQuery" type="text" :placeholder="$t('common.search')" class="input w-full"
		...
		<div v-else class="hidden md:block card overflow-hidden">
				...
				<tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
					<template v-if="groupsStore.groups.length > 0">
						<tr v-for="group in groupsStore.groups" :key="group.id"
							...
							<td class="px-6 py-4 whitespace-nowrap">
								<span
									:class="group.is_online ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400' : 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300'"
							...
							<td class="px-6 py-4 whitespace-nowrap">
								<span
									:class="group.is_active ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400' : 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400'"
									class="px-2 py-1 text-xs rounded-full">
			...
			<div v-if="groupsStore.groups.length > 0"
				...
				<div class="flex items-center gap-2">
					<button @click="groupsStore.setPage(groupsStore.currentPage - 1)"
						:disabled="groupsStore.currentPage === 1" class="btn-secondary px-3 py-1 disabled:opacity-50">
		...
		<!-- Create/Edit Modal -->
		<GroupFormModal v-if="showModal" :group="selectedGroup" :is-editing="isEditing" @close="closeModal"
			@created="onGroupCreated" @updated="onGroupUpdated" />

		<!-- Students Modal -->
		<GroupStudentsModal v-if="showStudentsModal && selectedGroup" :group="selectedGroup"
			@close="showStudentsModal = false" />

		<!-- Delete Confirmation Modal -->
		<ConfirmModal v-if="showDeleteModal" :title="$t('groups.deleteGroup')"
